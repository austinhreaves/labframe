...
describe('signAnswers', () => {
  ...
  it('maps 400 responses to invalid payload guidance', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValueOnce(mockJsonResponse({ error: 'Invalid body' }, 400)),
    );
  ...
  it('maps 413 responses to size guidance', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValueOnce(mockJsonResponse({ error: 'Payload too large' }, 413)),
    );
  ...
  it('maps 500 responses to server misconfiguration guidance', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValueOnce(mockJsonResponse({ error: 'Server misconfigured' }, 500)),
    );
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(
          mockJsonResponse({ signature: 'abcdef0123456789', signedAt: 1714450000000 }, 200),
        ),
    );
