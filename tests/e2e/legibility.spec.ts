...
async function assertUnpressedButtonLegible(page: Page) {
  await page.goto('/c/phy132/coulombsLaw');
  await expect(page.getByRole('heading', { name: /coulomb's law/i }).first()).toBeVisible();

  const unpressedButton = page.locator('button[aria-pressed="false"]').first();
  await expect(unpressedButton).toBeVisible();

  const computedColor = await unpressedButton.evaluate(
