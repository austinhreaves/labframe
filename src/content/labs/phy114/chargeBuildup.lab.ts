// PHY 114 variant of the PHY 132 Charge Buildup lab.
//
// Derived from phy132/chargeBuildup.draft.lab.ts. This PHY-114-owned copy adds
// the Pass 5 `parts` grouping (1A John Travoltage, 1B Balloons, 1C Two-Conductor
// Induction; Discussion + Conclusion are the sim-less review tail) and rewrites
// the three "select X from the simulation picker" lines, since the parts model
// binds each part to its simulation and removes the picker. Content is otherwise
// identical; PHY 114 carries no uncertainty content to strip here.
//
...
export const phy114ChargeBuildupLab: Lab = {
  id: 'chargeBuildup',
...
**Procedure step 1.** The simulation for this part is **John Travoltage**.
...
**Procedure step 1.** The simulation for this part is **Balloons and Static Electricity**.
...
**Procedure step 1.** The simulation for this part is **Two-Conductor Induction**.
...
---`,
  ...
  // Pass 5: each part owns one simulation. 1A and 1C use distinct sims here, so
  // the keep-alive win is revisiting (charge built in 1A survives a trip to 1B
  // and back). Sections [21, 25) are the sim-less review tail (Discussion and
  // Conclusion), surfaced in Finish & Review rather than parked behind a sim.
  parts: [
    {
      key: '1A',
      title: 'John Travoltage',
      simulationId: 'johnTravoltage',
      sectionRange: [0, 7],
    },
    {
      key: '1B',
      title: 'Balloons and Static Electricity',
      simulationId: 'balloons',
      sectionRange: [7, 14],
    },
    {
      key: '1C',
      title: 'Induction in Conductors',
      simulationId: 'twoConductorInduction',
      sectionRange: [14, 21],
    },
  ],
};
