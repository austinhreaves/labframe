...
// docs/handoffs/split-labs-3-7-handoff.md. Takes Parts 1A (E vs r, single
// point charge) and 1B (V vs r, single point charge) — the quantitative half
// of the source draft.
//
// Restructure decisions:
// - JIT theory delivery: Coulomb's Law + E-field surfaced before Part 1A;
//   electric potential + V-vs-1/r framing surfaced before Part 1B. Manual
//   Introduction equations (1)-(6) are the source for both.
...
//   convention.
// - Three <RecordTable> placeholders from the source draft ported to
//   dataTable schema blocks: a 4-direction sensor table for Part 1A(a);
//   a 2-row charge-doubling table for Part 1A(b); a 2-row distance-doubling
//   table for Part 1A(c).
// - Four ⚠️ TODO(human): missing prompt calculation blocks (sampleCalculation,
//   percentErrorCalculation, kqCalculation, slopeErrorCalculation) replaced
//   with real prompts drawn from the lab manual procedure.
// - Duplicate prompts on qualitativeObservations / potentialDistanceResponse /
//   inverseDistanceResponse de-duplicated.
// - Em dashes stripped from HTML/preamble strings; paragraph breaks added
//   inside long blocks per Mayer's spatial-contiguity principle.
...
export const phy132PointChargeLab: Lab = {
  id: 'pointCharge',
...
**Procedure step 2.** Click the equipotential tool (the meter with crosshairs). Move it to a point 0.5 m from the charge and click the pencil icon to draw the equipotential ring through that point. Repeat at 1.0, 1.5, 2.0, and 2.5 m. Record the potential value displayed by the meter for each ring in the data table.`,
    ...
    },
    {
      kind: 'instructions',
      tocHidden: true,
      html: '**Procedure step 5.** Read the slope and its uncertainty from the proportional fit on the $V$-vs-$1/r$ plot.',
    },
    {
      kind: 'multiMeasurement',
      rows: [
        { id: 'slopeValue', label: 'Slope m =' },
        { id: 'slopeUncertainty', label: 'Uncertainty σₘ =' },
      ],
      points: 0.5,
    },
    {
