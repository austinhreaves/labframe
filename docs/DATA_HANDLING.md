...
LabFrame is a single-page web application built in React + TypeScript with a single Vercel serverless function for signing. It is deployed at a URL under ASU control (production URL to be confirmed for v1.0). The source code is maintained internally and is available for review on request.
