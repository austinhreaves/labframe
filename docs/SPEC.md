...
- A Process Record appendix reports per-section activity. **[Decided 2026-06-10, not implemented]**: the grading PDF reports active time in coarse bands rather than raw milliseconds, and IME paste counts are excluded from the printed appendix (they already carry no integrity signal and flag CJK typists). Granular numbers belong in the inspector, not the grading surface.
- **[Decided 2026-06-30, not implemented]** Process Record rows carry a section-specific label, not the generic kind name. Today every row derives from `sectionTitle()`, which returns the section _kind_ ("Response" for every `concept` section, and `sectionTitle()` is shared with the PDF body headings and the compaction list, so it must not be edited in place). Add a dedicated `processRecordLabel(section)` used only by the appendix that prefers real identity: the section `title` when the schema carries one (plot / dataTable / image), else a truncated `prompt` (concept sections have `prompt` but no `title`), else the kind name, with a disambiguating index so repeats stay distinct (`Response 3`). Presentation only; the label is derived from the already-signed section, never a new signed field. Ship together with the bands + IME-exclusion change above (same appendix, same `Document.tsx`, same one-pass PDF-export verification).
- **Precondition:** the active-time metric must be leak-proof first (see [ACTIVE_TIME_ACCRUAL_SPEC](specs/ACTIVE_TIME_ACCRUAL_SPEC.md)); banding an `activeMs` that silently undercounts on reload would print misleading bands.
...
| 14  | Process Record: time bands instead of raw ms; exclude IME counts; section-specific row labels via `processRecordLabel` (not the generic kind name)                                                                                                 | Integrity   |
